...
class PytorchModel(nn.Module, BaseEstimator):
    def __init__(self, learning_rate=0.001, epochs=100, batch_size=50, random_state=0, device='cpu'):
        ...
        self.param_search = True  # 默认开启搜索参数功能，常规训练时必须设置为False
    ...
    def to_tensor(self, data):
        if data is None or isinstance(data, torch.Tensor) or isinstance(data, list) and not isinstance(data[0], np.ndarray):
            return data
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for epoch in pbar:
            if self.shuffle_every_epoch:  # 打乱训练集
    ...
    def fit_step(self, X, y=None, train=True, epoch=0):
        ...
        total_loss, y_hat = 0, []
        indexList = range(0, len(X[0]) if isinstance(X, list) and (isinstance(X[0], torch.Tensor) or isinstance(X[0], list)) else len(X), self.batch_size)
        if self.watch_epoch:
        ...
        for i in indexList:
            if isinstance(X, list):
                if isinstance(X[0], torch.Tensor):
                    ...
                    X_batch = [x[i: i + self.batch_size] for x in X]
                else:
    ...
    def predict_output(self, X, batch_size, output_all_value=False):
        ...
        X = self.to_tensor(X)
        y_hat = []
        indexList = range(0, len(X[0]) if isinstance(X, list) and (isinstance(X[0], torch.Tensor) or isinstance(X[0], list)) else len(X), batch_size)
        if self.watch_epoch:
